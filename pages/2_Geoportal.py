...
with open("auth_config.yaml", "r", encoding="utf-8") as _f:
    _cfg = yaml.load(_f, Loader=SafeLoader)

_auth = stauth.Authenticate(
    _cfg["credentials"], _cfg["cookie"]["name"], _cfg["cookie"]["key"], _cfg["cookie"]["expiry_days"]
)
...
# mostra botão Sair na sidebar (compatível com versões diferentes da lib)
try:
...
def build_report_pdf(site, date, taxa, inc, vento, img_url, fig1, fig2,
    ...
    def start_page():
        ...
        c.setFillColorRGB(0, 0, 0); c.setStrokeColorRGB(*ACCENT); c.setLineWidth(1)
    ...
    if fig2 is not None:
        try:
            ...
            if y - h2 < margin + 30: c.showPage(); y = start_page()
...
satellite = getv("Satelite", "Satélite", "Satellite", "Sat")
